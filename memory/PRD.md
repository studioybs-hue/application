...
- **2026-09-13 18:30** : bouton « Envoyer mes photos (parcourir) » toujours proposé aux mariés (même avec galerie) ; `POST /projects/{cid}/selection` sans `photo_ids` conserve les cœurs existants (count = cœurs + uploads). **Relance automatique** (`send_selection_reminders`, boucle toutes les 6 h, `POST /admin/projects/selection-reminders/run` pour test) : photos dispo ≥ 7 j sans sélection validée → SMS Brevo + email, tous les 7 j, 3 max, historisé dans `deliverables.selection.reminders`. Déployé. Lien de livraison d'exemple ajouté au suivi `yassina`.
- **SMTP IONOS** : `mailer._get_cfg_env` déduit désormais STARTTLS pour 587 (SSL implicite seulement pour 465 ou `SMTP_USE_SSL=true`) → la connexion à smtp.ionos.fr:587 fonctionne, mais **535 Authentication credentials invalid** persiste avec le mot de passe fourni par l'utilisateur (mis dans `.env` prod, sauvegarde `/root/backups/env.<ts>`). À vérifier côté IONOS : accès externe réellement activé/propagé, mot de passe exact. Test rapide : script smtplib sur le VPS.
- **Pré-vol Publish (2026-09-13)** : `qrcode==8.2` ajouté à requirements.txt (import guestbook.py) ; plus de mot de passe admin par défaut dans server.py (`ADMIN_PASSWORD` requis, sinon seed ignoré ; local .env contient ADMIN_PASSWORD=Admin13!) ; `.env` retiré du .gitignore (exigence pipeline Emergent). Volontairement conservé : `baseUrl.ts` force https://cinemaries.fr en build natif (le backend Emergent n'est PAS le backend de prod) ; eas.json et flux push existants inchangés. server.py prod non resynchronisé pour ce petit changement (à faire au prochain déploiement).
- **2026-09-13 18:50 (déployé)** : profil — l'admin ne voit plus l'aperçu d'un suivi client (`admin_preview` → project null) ; contenu centré `maxWidth: 720` sur grand écran. server.py prod resynchronisé (ADMIN_PASSWORD sans défaut).
