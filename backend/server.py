...
def _my_cid(user: dict) -> Optional[str]:
    """Mariage du client : assigné par l'admin (client_id) ou revendiqué (claimed_client_id)."""
...
def _owns_wedding(user: Optional[dict], client_id: str) -> bool:
    ...
    if not user or not client_id:
        return False
    return client_id in {user.get("client_id"), user.get("claimed_client_id")}
...
# Cache for ffmpeg-generated thumbnails + preview clips
FTP_CACHE_DIR = UPLOAD_DIR / ".ftp_cache"
FTP_CACHE_DIR.mkdir(parents=True, exist_ok=True)

_FTP_SIGN_SECRET = os.getenv("FTP_PREVIEW_SECRET", os.getenv("JWT_SECRET", "cinemaries-ftp-secret"))


def _sign_ftp_token(filename: str, exp_ts: int) -> str:
    msg = f"{filename}|{exp_ts}".encode("utf-8")
    sig = _hmac_mod.new(_FTP_SIGN_SECRET.encode("utf-8"), msg, _hashlib_mod.sha256).hexdigest()[:24]
    return f"{exp_ts}.{sig}"


def _verify_ftp_token(filename: str, token: str) -> bool:
    try:
        exp_str, sig = token.split(".", 1)
        exp_ts = int(exp_str)
        if exp_ts < int(datetime.now(timezone.utc).timestamp()):
            return False
        expected = _sign_ftp_token(filename, exp_ts)
...
def _ffmpeg_available() -> bool:
    try:
        r = _subprocess_mod.run(["ffmpeg", "-version"], capture_output=True, timeout=5)
        return r.returncode == 0
    except Exception:
        return False


def _ftp_cache_key(filename: str, kind: str) -> Path:
    safe = _hashlib_mod.sha256(filename.encode("utf-8")).hexdigest()[:16]
    ext = "jpg" if kind == "thumb" else "mp4"
    return FTP_CACHE_DIR / f"{safe}.{kind}.{ext}"


@api_router.get("/admin/ftp-files/thumbnail/{filename}")
async def ftp_thumbnail(filename: str, admin: dict = Depends(get_current_admin)):
    """Génère et retourne une miniature JPEG du fichier ftp_drop (cachée)."""
    if "/" in filename or "\\" in filename or ".." in filename:
        raise HTTPException(status_code=400, detail="Nom invalide")
    src = FTP_DROP_DIR / filename
    if not src.exists():
        raise HTTPException(status_code=404, detail="Fichier introuvable")
    cache = _ftp_cache_key(filename, "thumb")
    ext = src.suffix.lower().lstrip(".")
    image_exts = {"jpg", "jpeg", "png", "webp", "gif"}
    if ext in image_exts:
        return FileResponse(str(src), media_type=f"image/{'jpeg' if ext == 'jpg' else ext}")
    if not cache.exists():
        if not _ffmpeg_available():
            raise HTTPException(status_code=501, detail="ffmpeg requis pour les miniatures vidéo")
        try:
            _subprocess_mod.run([
                "ffmpeg", "-y", "-ss", "5", "-i", str(src),
                "-frames:v", "1", "-vf", "scale=320:-2",
                "-q:v", "5", str(cache),
            ], capture_output=True, timeout=30, check=True)
        except Exception as e:
            raise HTTPException(status_code=500, detail=f"Erreur génération miniature: {e}")
    return FileResponse(str(cache), media_type="image/jpeg")


@api_router.post("/admin/ftp-files/preview-token/{filename}")
async def ftp_preview_token(filename: str, admin: dict = Depends(get_current_admin)):
    """Génère un token signé court (10 min) pour lire le clip d'aperçu sans header auth."""
    if "/" in filename or "\\" in filename or ".." in filename:
        raise HTTPException(status_code=400, detail="Nom invalide")
    if not (FTP_DROP_DIR / filename).exists():
        raise HTTPException(status_code=404, detail="Fichier introuvable")
    exp_ts = int(datetime.now(timezone.utc).timestamp()) + 600  # 10 min
    token = _sign_ftp_token(filename, exp_ts)
    return {"token": token, "expires_in": 600}


@api_router.get("/admin/ftp-files/preview/{filename}")
async def ftp_preview(filename: str, token: str):
    """Sert un clip d'aperçu de 15s (cache) - authentifié par token signé en query param."""
    if "/" in filename or "\\" in filename or ".." in filename:
        raise HTTPException(status_code=400, detail="Nom invalide")
    if not _verify_ftp_token(filename, token):
        raise HTTPException(status_code=403, detail="Token invalide ou expiré")
    src = FTP_DROP_DIR / filename
    if not src.exists():
        raise HTTPException(status_code=404, detail="Fichier introuvable")
    cache = _ftp_cache_key(filename, "preview")
    if not cache.exists():
        if not _ffmpeg_available():
            raise HTTPException(status_code=501, detail="ffmpeg requis pour l'aperçu vidéo")
        try:
            _subprocess_mod.run([
                "ffmpeg", "-y", "-ss", "30", "-i", str(src),
                "-t", "15", "-vf", "scale=640:-2",
            ...
            raise HTTPException(status_code=500, detail=f"Erreur génération aperçu: {e}")
...
async def get_wedding(client_id: str, code: Optional[str] = None, current: Optional[dict] = Depends(get_optional_user)):
    ...
    if not filtered:
        raise HTTPException(status_code=404, detail="Mariage introuvable")
    # check unlock status — Premium ne débloque QUE le mariage du client (revendiqué/assigné)
    unlocked = False
    if current:
        if current.get("is_admin") or (current.get("is_subscribed") and _owns_wedding(current, client_id)):
            unlocked = True
        else:
...
async def unlock_wedding(body: UnlockRequest, request: Request, current: Optional[dict] = Depends(get_optional_user)):
    ...
    if current:
        ...
        if (current.get("is_subscribed") or current.get("is_admin")) and not current.get("client_id") and not current.get("claimed_client_id"):
...
async def client_list_codes(current: dict = Depends(get_current_user)):
    ...
    codes = await db.unlock_codes.find(
        {"client_id": _my_cid(current), "owner_user_id": current["id"]},
        {"_id": 0}
...
async def client_create_code(body: ClientCodeCreate, current: dict = Depends(get_current_user)):
    ...
    matching = [v for v in all_v if (v.get("client_id") or slugify(v.get("title", ""))) == _my_cid(current)]
    if not matching:
        raise HTTPException(status_code=404, detail="Mariage introuvable")
    ...
    return {"ok": True, "code": code, "client_id": _my_cid(current)}
...
async def get_video(video_id: str, code: Optional[str] = None, current: Optional[dict] = Depends(get_optional_user)):
    ...
    if current and v.get("is_showcase"):
        unlocked = True
    # 0bis) User owns this wedding (claimed/assigned) AND is subscribed → full access (the bride/groom)
    if not unlocked and current and current.get("is_subscribed"):
        v_cid = v.get("client_id") or slugify(v.get("title", ""))
        if _owns_wedding(current, v_cid):
            unlocked = True
    # 1) Logged-in user with a recorded unlock OR an admin (Premium seul ne débloque PAS les autres mariages)
    if not unlocked and current:
        u_doc = await db.user_unlocks.find_one({"user_id": current["id"], "video_id": video_id})
        unlocked = bool(u_doc) or bool(current.get("is_admin"))
